...
DAN.profile['dm_name']='LCY_Dummy'   # you can change this but should also add the DM in server
DAN.profile['df_list']=['Dummy_Sensor', 'Color-I', 'Dummy_Control']   # Check IoTtalk to see what IDF/ODF the DM has
DAN.profile['d_name']= '1D-LCY.IN' # None
...
gotInput=False
theInput=[0 for i in range(4)] # r, g, b, lum
allDead=False
...
def doRead( ):
    ...
    while True:   
        ...
        try:     # 準備讀取資料, 注意程式會卡在這等 User 輸入, 所以要用 Thread
            for i in range(4):
                theInput[i] = int(input("Give me data: "))
...
while True:
    ...
    #Push data to a device feature called "Dummy_Sensor" 
        ...
        if gotInput:
            ...
            DAN.push ('Dummy_Sensor', theInput[3],  theInput[3])  #  試這:  DAN.push('Dummy_Sensor', theInput) 
